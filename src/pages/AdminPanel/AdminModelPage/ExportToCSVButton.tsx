...
const ExportToCSVButton: React.FC<ExportToCSVButtonProps> = ({ data, className }) => {
	...
	const convertToCSV = (models: GetAllModelResponse[]): string => {
		...
		sortedModels.forEach(({ id, name, brand, createdDate, updatedDate }) => {
			...
			csvRows.push(`${id},${name.replace(',', ' ')},${brand.name.replace(',', ' ')},${formattedCreatedDate},${formattedUpdatedDate}`);
		});
		return csvRows.join('\n');
	};
    ...
    return (
        <Button className={`button-admin-model mb-2 ms-1 bg-warning ${className}`} onClick={handleExportClick}>Dışa Aktar</Button>
    );
};
